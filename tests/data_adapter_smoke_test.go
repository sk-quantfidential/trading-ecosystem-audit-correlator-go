...
func TestDataAdapterSmoke(t *testing.T) {
	...
	os.Setenv("POSTGRES_URL", "postgres://audit_adapter:audit-adapter-db-pass@localhost:5432/trading_ecosystem?sslmode=disable")
	os.Setenv("REDIS_URL", "redis://audit-adapter:audit-pass@localhost:6379/0")
	...
	t.Run("adapter_initialization", func(t *testing.T) {
		...
		if adapter.AuditEventRepository() == nil {
			t.Error("Expected AuditEventRepository to be non-nil")
		}
	...
	t.Run("audit_event_repository_basic_crud", func(t *testing.T) {
		// Audit event repository requires UUID generation enhancement - deferred to future epic
		t.Skip("Audit event repository requires UUID generation enhancement - deferred to future epic")
	})

	t.Run("service_discovery_smoke", func(t *testing.T) {
		// Requires Redis ACL permissions (keys, scan) for audit-adapter user
	...
	t.Run("cache_repository_smoke", func(t *testing.T) {
		...
		testKey := "audit:smoke-test:" + time.Now().Format("20060102150405")

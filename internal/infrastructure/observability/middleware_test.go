...
func TestREDMetricsMiddleware(t *testing.T) {
	t.Run("instruments_successful_requests_with_RED_metrics", func(t *testing.T) {
		...
		if !strings.Contains(metricsOutput, `service="audit-correlator"`) {
			t.Error("Expected service constant label in metrics")
		}
		if !strings.Contains(metricsOutput, `instance="audit-correlator"`) {
	...
	t.Run("uses_route_pattern_not_full_path_for_low_cardinality", func(t *testing.T) {
		...
		router.GET("/api/v1/events/:id", func(c *gin.Context) {
		...
		if !strings.Contains(metricsOutput, `route="/api/v1/events/:id"`) {
			t.Error("Expected route pattern /api/v1/events/:id in metrics (low cardinality)")
		}

		// Should NOT have full paths (high cardinality)
		if strings.Contains(metricsOutput, `route="/api/v1/events/123"`) {
			t.Error("Metrics should not contain full path /api/v1/events/123 (high cardinality)")
		}
	})
